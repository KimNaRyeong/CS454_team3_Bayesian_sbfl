...
def create_bayesian_network(pdg, coverage_df, failing_tests):
    ...
    for node in pdg.nodes:
        ...
        if len(successors) == 0: # 고립된 노드
            ...
            #     prob = 1
            # prob = a / total_test
            bayesian_network.add_node(node, failure_probability = 1)
            continue
...
# Paths
spectrum_file = './test_spectrum.json'
...
for chart_key, methods in spectrum_data.items():
    ...
    expected_file_name = f"{chart_key}_dependency_graph2.dot"


    pdg = read_pdg(expected_file_name)
    bug_info_path = "test_bug2.json"
    with open(bug_info_path, "r") as f:
        bug_info = json.load(f)


    coverage_path = "./test_data2.pkl"
